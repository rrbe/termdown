...
Tags are **not** covered by the branch-protection rule (it guards `refs/heads/master` only), so the tag push in step 7 succeeds directly — this is the one push that doesn't go through a PR. The `release.yml` workflow triggers on `v*` tag pushes and produces the GitHub Release (cross-platform binaries + checksums/installers), while `publish-crate.yml` publishes to crates.io (`cargo publish`) from the same tag. Pushing the tag is therefore the release trigger; the version stamp itself lands earlier via the bump PR.
